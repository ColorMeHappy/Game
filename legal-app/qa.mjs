...
if(idx.schemaVersion!=='5.1.0'||idx.contentRelease!=='2026.08.16.2'||idx.runtimeVersion!=='22')bad('app-index versioning');else ok('app-index versioning');
if('version'in idx||'contentVersion'in idx)bad('ambiguous app-index version keys');
const lessonIds=idx.paths.flatMap(p=>p.lessonIds);if(lessonIds.length!==20||new Set(lessonIds).size!==20)bad('20 unique lessons');else ok('20 lessons');
...
const state=read('js/state.js'),pages=read('js/pages.js'),lesson=read('js/lesson.js'),app=read('app.js'),sw=read('service-worker.js'),data=read('js/data.js'),manifest=json('manifest.webmanifest'),trail=read('js/skill-evidence-trail.js'),hub=read('js/professional-hub.js');
...
if(!idx.professionalPlatform||!hub.includes('Professional Simulation')||!hub.includes('Cold Case')||!hub.includes('Legal Sparring / Judge'))bad('professional platform integration');else ok('professional platform integration');
if(!sw.includes("const VERSION='v22'"))bad('service worker v22');else ok('service worker v22');
