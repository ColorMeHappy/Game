create unique index if not exists skill_evidence_practice_once_idx
...
begin
  ...
  if coalesce(trim(p_source_type),'') not in ('quiz','solve_stage','practice') then raise exception 'INVALID_SOURCE_TYPE'; end if;
  ...
  if v_inserted = 1 and p_confidence is not null then
    insert into public.confidence_events(
      event_id,user_id,source_type,source_id,correct,confidence
    ) values (
      p_event_id,v_user_id,trim(p_source_type),trim(p_source_id),p_raw_score >= 50,p_confidence
    )
