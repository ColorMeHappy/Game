...
try{await boot();migrateCourseV5(idx().legacyLessonMap||{});await render()}catch(e){console.error(e);appRoot.innerHTML=`<main style="padding:20px">${errorBox(e)}</main>`}
if('serviceWorker'in navigator){let refreshing=false;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(refreshing)return;refreshing=true;location.reload()});window.addEventListener('load',async()=>{try{const reg=await navigator.serviceWorker.register('./service-worker.js',{updateViaCache:'none'});await reg.update()}catch(e){console.error(e)}})}