let index=null;const catalogs=new Map(),lessons=new Map(),cases=new Map();let caseIndex=null,updates=null,searchData=null;
...
export async function casesList(){if(caseIndex)return caseIndex;const out=[];for(const f of index.caseIndexFiles){const d=await get(`./${f}`);for(const c of d.cases){c.linkedLessons=[...new Set((c.linkedLessons||[]).map(mapLessonId))];out.push(c)}}caseIndex=out;return out}
