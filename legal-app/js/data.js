...
async function loadCaseBundle(file){if(caseBundles.has(file))return caseBundles.get(file);const d=normalizeMeta(await get(`./${file}`));for(const raw of d.cases||[]){const c=normalizeMeta({...raw,linkedLessons:[...new Set((raw.linkedLessons||[]).map(mapLessonId))]});cases.set(c.id,c)}caseBundles.set(file,d);return d}
